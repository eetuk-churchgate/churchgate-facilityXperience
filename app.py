                    ...
                    try:
                        ...
                        class HDReportPDF(FPDF):
                            def header(self):
                                ...
                                self.set_xy(18,24)
                                self.cell(260,5,'Helpdesk Analytics Report',0,0,'L')
                                self.set_font('Helvetica','',8)
                                self.set_xy(18,29)
                                self.cell(260,5,f'{safe_text(info.get("full_name",fc))} | {rpt_month} {rpt_year}',0,0,'L')
                                self.set_y(40)
                            def footer(self):
                                self.set_y(-18)
                                self.set_font('Helvetica','I',7)
                ...
                if cat_id:
                    ...
                    with st.form("esc_form"):
                        for level in range(1, 7):
                            ...
                            with c1: st.multiselect(f"Users L{level}", user_options, key=f"esc_u_{level}_{cat_id}")
